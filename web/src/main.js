...
const appBasePath = window.location.pathname === '/devbrief' ||
  window.location.pathname.startsWith('/devbrief/') ? '/devbrief' : ''
const appUrl = (path) => `${appBasePath}${path.startsWith('/') ? path : `/${path}`}`

const prefixStaticAssets = () => {
  if (!appBasePath) return
  document.querySelectorAll('img[src^="/"]').forEach((image) => {
    if (!image.src.includes(`${window.location.host}${appBasePath}/`)) {
...
createApp({
  setup() {
    ...
    const speak = async () => {
      ...
      try {
        const response = await fetch(appUrl('/api/speak'), { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ text: speech.value }) })
    ...
    onMounted(async () => {
      ...
      const assetObserver = new MutationObserver(prefixStaticAssets)
    ...
    return { approve, approver, assetUrl: appUrl, audioUrl, busy, busyAction, canApprove, canRecover, candidates, checkpoints, completedRuns, current, error, evidence, evidencePath, evidenceResult, executionPlan, fileInput, formatDate, formatValue, health, labelState, loadRuns, navigate, navItems, notice, page, path, pendingRuns, receipt, recover, route, run, runs, selectRun, speak, speech, state, stateClass, trace }
  },
  template: `
    <div class="app-shell">
      <aside class="sidebar">
        <a class="brand" href="#/home" aria-label="DevBrief Agent 总览"><img :src="assetUrl('/devbrief-mark.svg')" alt="" class="brand-mark"><span class="brand-copy"><strong>DevBrief</strong><small>Agent workspace</small></span></a>
